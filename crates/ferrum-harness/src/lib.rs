...
use ferrum_types::event::{Envelope, Event, PermDecision};
use ferrum_types::model::{StopReason, Usage};
use ferrum_types::{CallId, Json, SessionId};
...
/// Where events live. PG in cloud, SQLite/file in `ferrum local`; in-memory
...
mod tests {
    use super::*;
    use ferrum_types::event::{ClientKind, ReducedOutput};
    use ferrum_types::model::{ContentBlock, ModelRef};
    use ferrum_types::TurnId;
