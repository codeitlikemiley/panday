...
use ferrum_types::{AccountId, Json, Timestamp};
...
impl ModelPrice {
    /// Usage convention (see `ferrum_types::model::Usage`): cache counts are
    /// SUBSETS of `input_tokens`; the fresh remainder prices at 1x.
    pub fn cost_micros(&self, u: &ferrum_types::model::Usage) -> u64 {
