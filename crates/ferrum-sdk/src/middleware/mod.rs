            ...
            match self.inner.chat(req.clone()).await {
                ...
                Err(e) => {
                    ...
                    let wait = match &e {
                        ...
                        FerrumError::RateLimited { retry_after_ms } if *retry_after_ms > 0 => {
...
impl<C: ModelClient> ModelClient for Timeout<C> {
    async fn chat(&self, req: ChatRequest) -> Result<ItemStream, FerrumError> {
        match tokio::time::timeout(self.limit, self.inner.chat(req)).await {
            ...
            Err(_) => Err(FerrumError::Provider {
...
mod tests {
    ...
    struct Flaky {
        fail_times: u32,
        error: fn() -> FerrumError,
        attempts: Arc<AtomicU32>,
    }

    #[async_trait::async_trait]
    impl ModelClient for Flaky {
        async fn chat(&self, _req: ChatRequest) -> Result<ItemStream, FerrumError> {
    ...
    fn flaky(fail_times: u32, error: fn() -> FerrumError) -> (Flaky, Arc<AtomicU32>) {
    ...
    fn fatal() -> FerrumError {
        FerrumError::EntitlementDenied {
            plan: "free".into(),
    ...
    async fn never_retries_a_non_retryable_error() {
        ...
        let err = client.chat(a_request()).await.map(|_| ()).unwrap_err();
        assert!(matches!(err, FerrumError::EntitlementDenied { .. }));
    ...
    async fn a_server_stated_retry_after_wins_over_our_curve() {
        ...
        let (inner, attempts) = flaky(1, || FerrumError::RateLimited {
