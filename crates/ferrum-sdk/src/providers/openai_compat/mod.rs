...
impl ChunkTranslator {
    ...
    pub fn push(&mut self, record: &str) -> Result<Vec<StreamItem>, FerrumError> {
        ...
        let chunk: WireChunk = serde_json::from_str(record).map_err(|e| {
            FerrumError::Protocol(format!("openai_compat: malformed stream chunk: {e}"))
        })?;
...
fn into_items(
...
) -> impl futures_core::Stream<Item = Result<StreamItem, FerrumError>> + Send {
    ...
    struct State {
        ...
        pending: std::collections::VecDeque<Result<StreamItem, FerrumError>>,
        done: bool,
    }
...
"#,
    ...
    fn malformed_json_is_a_protocol_error_not_a_panic() {
        ...
        assert!(matches!(err, FerrumError::Protocol(_)));
    ...
    impl MockTransport {
        ...
        fn failing(err: FerrumError) -> Arc<Self> {
            Arc::new(Self {
                chunks: Vec::new(),
                fail: Some(err),
                seen: Mutex::new(None),
        ...
        async fn post_sse(
            ...
            *self.seen.lock().unwrap() = Some(SeenRequest {
            ...
            if let Some(e) = &self.fail {
                // FerrumError is not Clone; rebuild the shape under test.
                return Err(match e {
                    FerrumError::RateLimited { retry_after_ms } => FerrumError::RateLimited {
                        retry_after_ms: *retry_after_ms,
                    },
                    other => FerrumError::Protocol(other.to_string()),
                });
            }
            let chunks: Vec<Result<Vec<u8>, FerrumError>> =
    ...
    fn a_request() -> ChatRequest {
        ...
        use ferrum_types::model::{CallMeta, ContentBlock, Message, ModelRef, Role, Sampling};
        ChatRequest {
