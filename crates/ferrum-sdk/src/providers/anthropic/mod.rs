...
impl EventTranslator {
    ...
    pub fn push(&mut self, record: &str) -> Result<Vec<StreamItem>, FerrumError> {
        ...
        let event: WireEvent = serde_json::from_str(record).map_err(|e| {
            FerrumError::Protocol(format!("anthropic: malformed stream event: {e}"))
        })?;
        ...
        match event {
            ...
            WireEvent::Error { error } => {
                return Err(FerrumError::Provider {
...
impl ModelClient for AnthropicClient {
    async fn chat(&self, req: ChatRequest) -> Result<ItemStream, FerrumError> {
        let body = serde_json::to_vec(&wire::WireRequest::from_ir(&req))
            .map_err(|e| FerrumError::Protocol(format!("anthropic: encode request: {e}")))?;
