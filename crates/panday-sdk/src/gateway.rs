...
//!
//! Everything our own services send to a model goes through `panday-gateway`
//! (architecture sentence 3: "Every model call in the company goes through
//! panday-gateway ... *because* it is the only door"). This is the client side
//! of that door.
//!
...
//! the base URL and that the bearer token is a panday API key.
...
/// A client for a `panday-gateway` instance.
...
impl GatewayTransport {
    ...
    /// `api_key` is a panday key (`pnd_live_…`), not a provider key — the
...
"#;
    ...
    async fn streams_a_completion_end_to_end_through_the_gateway() {
        ...
        assert_eq!(key.as_deref(), Some("pnd_live_abc"));
    }
