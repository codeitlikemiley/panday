...
//!
//! ```text
//! PANDAY_UPDATE_GOLDEN=1 cargo test -p panday-types --test golden
//! ```

use panday_types::event::{
    Actor, ClientKind, Envelope, Event, PermDecision, ReducedOutput, KNOWN_EVENT_TAGS,
};
use panday_types::id::{AccountId, ArtifactRef, CallId, RequestId, SessionId, TurnId};
use panday_types::model::{ContentBlock, ModelRef, StopReason, Usage};
use panday_types::{Timestamp, PROTOCOL_VERSION};
        ...
        (
            ...
            Envelope {
                ...
                ..env(Event::ToolCall {
                    call_id: call(),
                    tool: "run_tests".into(),
                    args: serde_json::json!({ "package": "panday-types" }),
        ...
        (
            ...
            Envelope {
                ...
                ..env(Event::PermissionRequest {
                    call_id: call(),
                    tool: "edit_file".into(),
                    action: "write crates/panday-types/src/event.rs".into(),
...
fn updating() -> bool {
    std::env::var_os("PANDAY_UPDATE_GOLDEN").is_some()
}
